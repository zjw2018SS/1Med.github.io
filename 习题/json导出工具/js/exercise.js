...
async function clip_box_fun(clip_box_div){
    ...
    if(isJSON(text)){
        ...
        swal({
            title: "格式错误",
            text: "仅支持json格式，来源为学习通的油猴脚本（暂未开放）",
            icon: "error",
        })
    }
...
// 是否收藏逻辑
let e_main = document.getElementById("main")
e_main.addEventListener("click", is_favourite_fun)
...
function submit() {
    ...
    let swal_text = `共${correct_num + wrong_num}道题，答对${correct_num}道题，答错${wrong_num}道题，正确率为${(correct_num / (correct_num + wrong_num) * 100).toFixed(2)}%。
`
    let contentHtml = '<p style="color:blue" id="swal_html">小提示：点击页面左上侧功能球能提高您的学习效率</p>'

    swal({
        title: "恭喜您！",
        text: swal_text + "\n\n(tips：页面左上侧功能球能提高学习效率,手机上点击或电脑上鼠标悬停在正确答案上自动显示答案（可在功能球关闭）)",
        icon: "success",
    })
...
function json2paper(json, is_order = "1") {
    // console.log("json2paper",typeof json)
    try {
    ...
    if (json && json["head"].hasOwnProperty("title")) {
        ...
        title.style.margin = "15px auto "
        title.style.padding = "0px 15px 0px"
...
function is_hide_answer_true_fun(is_hide_answer_true_input) {
    ...
    let is_option_right_display = document.getElementById("option_before").dataset.isOptionRightDisplay
    // console.log(is_option_right_display);
