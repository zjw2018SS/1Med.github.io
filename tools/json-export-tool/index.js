...
let g_rules_default = []
let g_rules = {}
const g_rules_id = ["wrap_re", "type_code_input_re", "type_re", "q_re", "options_wrap_re", "option_re", "answer_true_re", "analysis_re", "answer_true_row_input_re"]
const g_head_info_id = ["head_info_author", "head_info_filename", "head_info_course", "head_info_source"]
let g_output_rules_json = {}
...
function textarea1_preview() {
    ...
    if (isJSON(textarea1_value)) {
        ...
        localStorage.setItem("1med:tool-preview-bank", json_str)
        const title = json_temp?.head?.filename || json_temp?.head?.course || "工具预览题库"
        window.open(`../../#/practice/exercise?toolPreview=1&title=${encodeURIComponent(title)}`, "_blank")
    } else {
...
let radio_input_re = document.querySelectorAll("#page1 input[type='radio']")
    ...
    if (wrap_re && wrap_re.test(all_value)) {
        ...
        for (let v = 0; v < wrap.length; v++) {
            ...
            if (type_code == "1" || type_code == "2") {
                ...
                let replace_contenteditable = document.querySelectorAll("#page1_right .replace_contenteditable")
                ...
                let replace_contenteditable = document.querySelector("#page1_right .replace_contenteditable")?.innerText || ""
